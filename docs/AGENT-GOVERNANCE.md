...
这几个字段的作用更接近“上下文预算和压缩策略”，而不是每次都强塞固定 token 数量。OpenClaw 实际运行时仍会结合模型真实 `contextWindow` 取更小值。
...
对于多账号 / 多 Agent / 多私聊入口，群准入和白名单并不能替代 DM 隔离。当前面板已把 `session.dmScope` 做成结构化可视化入口。
