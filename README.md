...
[![Pro](https://img.shields.io/badge/Pro-5.2.7-violet?style=flat-square)](https://github.com/zhaoxinyi02/ClawPanel/releases)
[![Lite](https://img.shields.io/badge/Lite-0.1.0-2563eb?style=flat-square)](https://github.com/zhaoxinyi02/ClawPanel/releases)
...
> [!NOTE]
> **v5.2.7 更新** — QQ (NapCat) 通道新增一键删除与更稳的账号级 OneBot 配置修复；插件型通道支持在通道页直接一键安装；插件仓库镜像补齐并新增 Gitee 回退，减少用户只看到部分插件的问题。详见 [changelogs/v5.2.7.md](changelogs/v5.2.7.md)

> [!TIP]
> **ClawPanel Lite v0.1.0** 已可用：Linux 安装后直接内置 OpenClaw 2026.2.26 和常用通道插件，用户只需配置 API Key 与通道凭据即可使用。详见 [changelogs/lite-v0.1.0.md](changelogs/lite-v0.1.0.md)
...
| 版本 | 适合谁 | 运行时策略 | 安装方式 |
|:---|:---|:---|:---|
| `ClawPanel Lite` | 新手 / 想直接可用 | 自带并托管 OpenClaw，不支持外部接管 | 安装后直接配置 API Key 与通道 |
| `ClawPanel Pro` | 已有 OpenClaw 环境 / 进阶用户 | 保持当前纯面板模式，支持外部实例接管与一键安装 OpenClaw | 按需安装运行时和插件 |
