...
func GetFeishuDMDiagnosis(cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"ok": true, "diagnosis": buildFeishuDMDiagnosis(cfg)})
	}
}
...
func buildFeishuDMDiagnosis(cfg *config.Config) FeishuDMDiagnosis {
	...
	if len(accountIDs) == 0 && (strings.TrimSpace(toString(feishuCfg["appId"])) != "" || strings.TrimSpace(toString(feishuCfg["appSecret"])) != "") {
...
func collectFeishuSessionKeys(sessionIndex map[string]interface{}) []string {
	...
	for key, raw := range sessionIndex {
		...
		if strings.TrimSpace(toString(getNestedMapValue(record, "deliveryContext", "channel"))) == "feishu" || strings.TrimSpace(toString(getNestedMapValue(record, "origin", "provider"))) == "feishu" {
