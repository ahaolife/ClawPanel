...
func TestSaveOpenClawConfigRejectsInvalidNumericFields(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			config: map[string]interface{}{
				"agents": map[string]interface{}{
					"defaults": map[string]interface{}{"contextTokens": 0},
				},
			},
			...
			config: map[string]interface{}{
				"agents": map[string]interface{}{
					"defaults": map[string]interface{}{
						"compaction": map[string]interface{}{"maxHistoryShare": 1.2},
					},
				},
			},
...
func TestSaveOpenClawConfigPreservesMissingHiddenFieldsWithoutOverwritingExplicitChanges(t *testing.T) {
	...
	initial := map[string]interface{}{
		"tools": map[string]interface{}{
			"profile":      "minimal",
			"agentToAgent": map[string]interface{}{"enabled": true},
		},
		"session": map[string]interface{}{
			"dmScope":     "main",
			"maintenance": map[string]interface{}{"maxEntries": 50},
		},
		"models": map[string]interface{}{"providers": map[string]interface{}{}},
	}
	raw, _ := json.Marshal(initial)
	if err := os.WriteFile(filepath.Join(dir, "openclaw.json"), raw, 0644); err != nil {
		t.Fatalf("write openclaw.json: %v", err)
	...
	body, _ := json.Marshal(map[string]interface{}{
		"config": map[string]interface{}{
			"tools":   map[string]interface{}{"profile": "full"},
			"session": map[string]interface{}{"dmScope": "per-account-channel-peer"},
			"models":  map[string]interface{}{"providers": map[string]interface{}{}},
		},
	})
	req := httptest.NewRequest(http.MethodPut, "/openclaw/config", bytes.NewReader(body))
...
func TestSaveOpenClawConfigRejectsInvalidDMScope(t *testing.T) {
	...
	body, _ := json.Marshal(map[string]interface{}{
		"config": map[string]interface{}{"session": map[string]interface{}{"dmScope": "user"}},
	})
	req := httptest.NewRequest(http.MethodPut, "/openclaw/config", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
...
func TestGetFeishuDMDiagnosisReportsSharedMainSession(t *testing.T) {
	...
	rawSessions, _ := json.Marshal(map[string]interface{}{
		"agent:main:main": map[string]interface{}{
			"deliveryContext": map[string]interface{}{"channel": "feishu", "accountId": "fly"},
		},
	})
...
func TestNormalizeFeishuChannelConfigMirrorsDefaultAccountToTopLevel(t *testing.T) {
	...
	input := map[string]interface{}{
		"defaultAccount": "main",
		"accounts": map[string]interface{}{
			"main":   map[string]interface{}{"appId": "cli_main", "appSecret": "secret_main"},
			"backup": map[string]interface{}{"appId": "cli_backup"},
		},
	}
...
func TestNormalizeFeishuChannelConfigSeedsDefaultAccountFromTopLevel(t *testing.T) {
	...
	input := map[string]interface{}{"appId": "cli_simple", "appSecret": "secret_simple", "defaultAccount": "default"}
	got := normalizeFeishuChannelConfig(input)
	...
	if entry["appId"] != "cli_simple" || entry["appSecret"] != "secret_simple" {
		t.Fatalf("expected default account to inherit top-level credentials, got %#v", entry)
	}
}
...
func TestNormalizeFeishuChannelConfigParsesMentionAndAllowList(t *testing.T) {
	...
	input := map[string]interface{}{"requireMention": "false", "groupPolicy": "allowlist", "groupAllowFrom": "oc_123， oc_456 , "}
	got := normalizeFeishuChannelConfig(input)

	if got["requireMention"] != false {
		t.Fatalf("expected requireMention false, got %#v", got["requireMention"])
	}
	groupAllowFrom, _ := got["groupAllowFrom"].([]interface{})
	if len(groupAllowFrom) != 2 || groupAllowFrom[0] != "oc_123" || groupAllowFrom[1] != "oc_456" {
		t.Fatalf("unexpected normalized allowlist: %#v", got["groupAllowFrom"])
	}
}
...
func TestNormalizeFeishuChannelConfigDropsAllowlistOutsideAllowlistPolicy(t *testing.T) {
	...
	input := map[string]interface{}{"groupPolicy": "open", "groupAllowFrom": "oc_123, oc_456"}
	got := normalizeFeishuChannelConfig(input)

	if _, exists := got["groupAllowFrom"]; exists {
		t.Fatalf("expected groupAllowFrom removed outside allowlist policy, got %#v", got["groupAllowFrom"])
	}
}
...
func TestNormalizeFeishuChannelConfigDropsLegacyDMScope(t *testing.T) {
	...
	input := map[string]interface{}{"dmScope": "user", "dmPolicy": "pairing"}
	got := normalizeFeishuChannelConfig(input)

	if _, exists := got["dmScope"]; exists {
		t.Fatalf("expected legacy dmScope removed, got %#v", got["dmScope"])
	}
	if got["dmPolicy"] != "pairing" {
		t.Fatalf("expected dmPolicy preserved, got %#v", got["dmPolicy"])
	}
}
