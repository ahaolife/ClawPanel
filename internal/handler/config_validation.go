...
func validateOpenClawNumericConfig(ocCfg map[string]interface{}) error {
	...
	rules := []struct {
	...
	}{
		{
			...
			constraint: numericFieldConstraint{label: "agents.defaults.contextTokens", integer: true, min: float64Ptr(1)},
		},
		{
			value:      getNestedMapValue(ocCfg, "agents", "defaults", "maxConcurrent"),
			constraint: numericFieldConstraint{label: "agents.defaults.maxConcurrent", integer: true, min: float64Ptr(1)},
		},
		{
			value:      getNestedMapValue(ocCfg, "agents", "defaults", "compaction", "maxHistoryShare"),
			constraint: numericFieldConstraint{label: "agents.defaults.compaction.maxHistoryShare", min: float64Ptr(0), max: float64Ptr(1)},
		},
		{
			value:      getNestedMapValue(ocCfg, "session", "maintenance", "maxEntries"),
			constraint: numericFieldConstraint{label: "session.maintenance.maxEntries", integer: true, min: float64Ptr(1)},
		},
		{
			value:      getNestedMapValue(ocCfg, "session", "agentToAgent", "maxPingPongTurns"),
			constraint: numericFieldConstraint{label: "session.agentToAgent.maxPingPongTurns", integer: true, min: float64Ptr(1)},
		},
		{
			value:      getNestedMapValue(ocCfg, "gateway", "port"),
			constraint: numericFieldConstraint{label: "gateway.port", integer: true, min: float64Ptr(1), max: float64Ptr(65535)},
		},
	}
	...
	return validateEnumField(getNestedMapValue(ocCfg, "session", "dmScope"), "session.dmScope", "main", "per-peer", "per-channel-peer", "per-account-channel-peer")
}
...
func validateAgentContextConfig(agent map[string]interface{}) error {
	...
	if err := validateNumericField(agent["contextTokens"], numericFieldConstraint{label: "contextTokens", integer: true, min: float64Ptr(1)}); err != nil {
		return err
	}
	compaction, _ := agent["compaction"].(map[string]interface{})
	return validateNumericField(getNestedMapValue(map[string]interface{}{"compaction": compaction}, "compaction", "maxHistoryShare"), numericFieldConstraint{label: "compaction.maxHistoryShare", min: float64Ptr(0), max: float64Ptr(1)})
}
